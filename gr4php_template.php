	...
	private static $possibleInputValues=array(
		":lax"=>array(
				...
				"title"=>array("{?uri rdfs:label ?title. FILTER(contains(lcase(str(?title)), lcase('","value","'))) .} UNION
					...
					{?uri dc:title ?title. FILTER(contains(str(?title), '","value","')) .}"),
				"country"=>array("{?uri vc:ADR ?adr} UNION {?uri vcard:adr ?adr}
					{?adr vc:Country ?country. FILTER(contains(str(?country), '","value","')) .} UNION 
					{?adr vcard:country-name ?country. FILTER(contains(str(?country), '","value","')) .}"),
				"street"=>array("{?uri vc:ADR ?adr} UNION {?uri vcard:adr ?adr}
					{?adr vc:Street ?street. FILTER(contains(str(?street), '","value","')) .} UNION 
					{?adr vcard:street-address ?street. FILTER(contains(str(?street), '","value","')) .}"),
				"post"=>array("{?uri vc:ADR ?adr} UNION {?uri vcard:adr ?adr}
					{?adr vc:Pcode ?post. FILTER(contains(str(?post), '","value","')) .} UNION
					{?adr vcard:postal-code ?post. FILTER(contains(str(?post), '","value","')) .}}"),
				"city"=>array("{?uri vc:ADR ?adr} UNION {?uri vcard:adr ?adr}
					{?adr vc:City ?city. FILTER(contains(str(?city), '","value","')) .} UNION
					{?adr vcard:locality ?city. FILTER(contains(str(?city), '","value","')) .}"),
				"legalName"=>array("?uri gr:legalName ?legalName. FILTER(contains(str(?legalName), '","value","')) ."),
				"duns"=>array("?uri gr:hasDUNS ?duns. FILTER(contains(str(?duns), '","value","')) ."),
				"isicv4"=>array("?uri gr:hasISICv4 ?isicv4. FILTER(contains(str(?isicv4), '","value","')) ."),
				"naics"=>array("?uri gr:hasNAICS ?naics. FILTER(contains(str(?naics), '","value","')) ."),
				...
				"sku"=>array("?uri gr:hasStockKeepingUnit ?sku . FILTER(contains(str(?sku), '","value","')) ."),
				"ean13"=>array("?uri gr:hasEAN_UCC-13 ?ean13. FILTER(contains(str(?ean13), '","value","')) ."),
				"gtin14"=>array("?uri gr:hasGTIN-14 ?gtin14. FILTER(contains(str(?gtin14), '","value","')) ."),
				"manufacturer"=>array("?uri gr:hasManufacturer ?manufacturer. FILTER(contains(str(?manufacturer), '","value","')) ."),
				...
				"validThrough"=>array("?uri gr:validThrough ?validThrough. FILTER(contains(str(?validThrough), '","value","')) ."),
				"validFrom"=>array("?uri gr:validFrom ?validFrom. FILTER(contains(str(?validFrom), '","value","')) ."),
				...
				"currency"=>array("?uri gr:hasPriceSpecification ?pricespec.
					?pricespec gr:hasCurrency ?currency. FILTER(contains(str(?currency), '","value","')) ."),
				"acceptedPaymentMethod"=>array("?uri gr:acceptedPaymentMethods ?acceptedPaymentMethod. FILTER(contains(str(?acceptedPaymentMethod), '","value","')) ."),
				"businessFunction"=>array("?uri gr:hasBusinessFunction ?businessFunction. FILTER(contains(str(?businessFunction), '","value","')) ."),
				...
				"eligibleCustomerTypes"=>array("?uri gr:eligibleCustomerTypes ?eligibleCustomerTypes. FILTER(contains(str(?eligibleCustomerTypes), '","value","')) ."),
				"eligibleRegions"=>array("?uri gr:eligibleRegions ?eligibleRegions. FILTER(contains(str(?eligibleRegions), '","value","')) ."),
				"availabilityStarts"=>array("?uri gr:availabilityStarts ?availabilityStarts. FILTER(contains(str(?availabilityStarts), '","value","')) ."),
				"availabilityEnds"=>array("?uri gr:availabilityEnds ?availabilityEnds. FILTER(contains(str(?availabilityEnds), '","value","')) ."),
				"availableDeliveryMethods"=>array("?uri gr:availableDeliveryMethods ?availabledeliveryMethods. FILTER(contains(str(?availabledeliveryMethods), '","value","')) ."),
